...
const Login = () => {
  ...
  const handleExistingGoogleUser = async (userData) => {
    try {
      ...
      const response = await axios.post("http://localhost:3001/api/user/get-user", { 
  ...
  // Decode JWT token (Keep this for potential future use)
  ...
  const handleOtpSubmit = async () => {
    if (otp.length === 6 && String(otp).trim() === String(sentOtp).trim()) {
      ...
      try {
        const response = await axios.post("http://localhost:3001/api/user/get-user", { email });
